    ...
    def dropEvent(self, event: QDropEvent):
        ...
        if files:
            ...
            if os.path.isdir(files[0]):
                ...
                if image_files:
                    self.display_image(image_files[0])  # Display the first image
                    self.drop_handler(image_files, self)  # Pass all images to the handler
                else:
                    QMessageBox.warning(self, "No Images", "The folder does not contain any supported image files.")
            else:
                self.display_image(files[0])  # Display the first image
                self.drop_handler(files, self)  # Pass all files to the handler
        event.acceptProposedAction()
